...
const DisplayStudents = ({ onStudentClick, filters = { searchTerm: '', selectedClass: '', selectedPosition: '' } }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto my-8">
      <ul className="bg-gradient-to-r from-gray-50 to-gray-100 shadow-lg overflow-auto rounded-lg max-h-96 grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
        {filteredStudents.map((student) => (
          <li key={student._id} className="bg-white transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl rounded-lg">
            <div className="flex flex-col md:flex-row justify-between items-center p-6">
              <div>
                <button onClick={() => onStudentClick(student)} className="inline-block bg-indigo-600 text-white text-lg px-3 py-1 rounded-full font-semibold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                  {student.name}
                </button>
                ...
                <span className="inline-block bg-indigo-200 text-indigo-800 text-md px-3 py-1 rounded-full uppercase font-semibold tracking-wide mt-2">{student.position}</span>
              </div>
              <div className="flex flex-col items-center mt-4 md:mt-0">
                ...
                <input 
                  ...
                  className="w-full cursor-pointer rounded-lg overflow-hidden appearance-none bg-indigo-100 h-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                />
